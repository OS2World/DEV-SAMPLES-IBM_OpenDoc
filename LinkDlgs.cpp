...
//
// @(#) 1.24 com/src/storage/LinkDlgs.cpp, oddataxfer, od96os2, odos29712d 3/7/97 11:22:59 [ 3/21/97 17:18:47 ]
...
ODStatic ISOString GetDisplayKind(ODType type, ODSession* session);
                          ...
                          ODSession* session, ODULong idItem)
{
  ...
  {
    ISOString   theName = GetDisplayKind(type, session);
    WinInsertLboxItem(hwndListBox, LIT_FIRST, theName);
    return;
  }
...
Environment* ev = somGetGlobalEnvironment();
   ...
   switch(msg)
   {
      case WM_INITDLG:
      {
            ...
            // set the Kind
            WinSetDlgItemText(hwnd, IDD_LDKINDTXT, GetDisplayKind(info->kind, gSession));
      ...
      case WM_INITDLG:
      {
            ...
            if (linkSource->Lock(ev, 0, &key))
            {
              TRY
                ...
                if (kind)  // was kindLen - rrk 28feb97
                    WinSetDlgItemText(hwnd, IDD_LSKINDTXT,GetDisplayKind(kind, gSession));
...
ODStatic ISOString GetDisplayKind(ODType type, ODSession* session)
{
   ...
   theName =  odRegistry->GetKindDisplayNameFromKind(ev, type);
   if ( (strcmp(theName, "GetKindDisplayName: No PartKind matches partKindName") != 0)
	&& (strcmp(theName, "GetKindDisplayName: No Parts found") != 0) ) {
        return(theName);
        }
   else {
        ...
        if ( platformType == kODNULL ) {
           return( catgets(ODcat_handle, LINKSPEC_SET, LINKSPEC_80, "No user kind"));
        }
        else {
#ifdef RRK_DEBUG
		   PRINT("RRK **** platformType is %d\t type is %s.\n", platformType, type);
#endif           
	  char    szUserKind[256];
	  memset(szUserKind,0,256);
	  ...
	  switch ( (UINT)platformType ) {
		...
		break;					
	    case CF_BITMAP:
		return(newString("BITMAP")); 
		break;					
	    case CF_DSPTEXT:
		return(newString("DSPTEXT"));
		break;					
   	    case CF_DSPBITMAP:
		return(newString("DSPBITMAP")); 
		break;					
	    case CF_METAFILE:
		return(newString("METAFILE"));
		break;					
	    case CF_DSPMETAFILE:
		return(newString("DSPMETAFILE"));
		break;					
	    case CF_PALETTE:
		return(newString("PALETTE"));
		break;					
            default:
		return( catgets(ODcat_handle, LINKSPEC_SET, LINKSPEC_80, "No user kind"));
#if 0
                if (!WinQueryAtomName(WinQuerySystemAtomTable(), platformType, szUserKind, 256)) {
