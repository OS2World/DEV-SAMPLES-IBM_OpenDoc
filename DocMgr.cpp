/* @(#)Z 1.102 com/src/docshell/DocMgr.cpp, odshell, od96os2, odos29712d 97/03/21 17:37:31 (97/03/05 14:33:48) */
					...
					ODUShort actionFlag)
{
	...
	somSelf->GetOleWrapper(ev)->RegisterFilename(fileName, FALSE);
        ...
        // Check if it is a read-only document. If so, display a
        // read-only alert dialog to ask user whether to continue
        // or not. If continue, then proceed to open the document.
        if (somSelf->IsReadOnlyDocument(ev, file))
        {
...
// IsReadOnlyDocument
//
//      Display an alert dialog to prompt user whether to continue to 
//	open the document or not if the document is read-only.
//------------------------------------------------------------------------------
SOM_Scope ODBoolean  SOMLINK DocumentManagerIsReadOnlyDocument(
                                                DocumentManager *somSelf,
                                                Environment *ev,
                                                PlatformFile *file)
{
    DocumentManagerData *somThis = DocumentManagerGetData(somSelf);
    DocumentManagerMethodDebug("DocumentManager",
                                	"DocumentManagerIsReadOnlyDocument");
    ...
    try
    {
        ...
        // Check if file is readonly. If so, display an alert dialog
        // and ask whether user wants to continue or not?
        file->GetFileAttributes(&fileAttrs);
        if (fileAttrs & kODFileAttrReadOnly)
           rc = ReadOnlyDlg(ev);
    } 
    ...
    {
        LOG("SOM exception occured in DocumentManager::IsReadOnlyDocument\n");
	ODSetSOMException(ev, _exception);
    }
