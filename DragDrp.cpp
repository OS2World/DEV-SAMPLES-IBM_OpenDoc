/* @(#)Z 1.19 os2/src/storage/DragDrp.cpp, oddataxfer, od96os2, odos29714c 97/04/02 17:20:18 (97/04/02 10:57:34) */
	...
	TRY
			 ...
			 theDragLink = (ODDragLink*) theIter->Next(), index++)
		{
			theSU = theDragLink->fItem->fSU;
                        ...
                        if(window) {
		  	   ODDraft *theDraft = window->GetDraft(ev);
	           	   if (theDraft) {
                           	ODBoolean isDraftReadOnly = (theDraft->GetPermissions(ev) == kODDPReadOnly);
                           	//PRINT("ExportClipboard: isDraftReadOnly is %d.\n", isDraftReadOnly);
                           	if (isDraftReadOnly)
			              	if ( theSU->Exists(ev, kODPropLinkSpec, kODNULL, 0) )
        				      ODSURemoveProperty(ev, theSU, kODPropLinkSpec);
			}
                           ODReleaseObject(ev,window);
                        }
