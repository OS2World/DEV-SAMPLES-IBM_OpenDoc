...
/* @(#)Z 1.11 com/src/imaging/realshps/PolyShpe.cpp, odimaging, od96os2, odos29712d 97/03/21 17:18:23 (97/02/05 14:20:43) */
...
#endif // IBM Platforms
{
   ...
   if( result.HasData() ) {
      ...
      // Simplify the polygon in the Union case.  This method will
      // get called when 2 RectShapes are unioned.  The PolyClip
      // method is not good at removing co-linear points so we
      // have to simplify it ourselves.
      if( op == kShapeUnion )
         Simplify( );
      this->Purge( 0 );
      return this;
