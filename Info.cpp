/* @(#)Z 1.26 os2/src/core/Info.cpp, odcore, od96os2, odos29712d 97/03/21 17:40:45 (97/03/12 10:11:26) */
...
typedef struct ODInfoDlgProgDataTag
{
                ...
                ODFacet* facet, ODBoolean allowEditing)
{
        ...
        // Allow editing if the part says so and the draft permissions allow it
        AllowEditing = allowEditing;
	if (AllowEditing)
            AllowEditing = su->GetDraft(ev)->GetPermissions(ev) 
                               >= kODDPSharedWrite;

	// Use Apple's technique of forcing a part's externalization so
	// that all necessary data is sure to exist in the SU
	TempODPart part = frame->AcquirePart(ev);
	if (su->GetDraft(ev)->GetPermissions(ev) >= kODDPSharedWrite)
	    part->Externalize(ev);
        ...
        result = CreatePropSheet(hwnd, facet);
...
int CreatePropSheet(HWND  hpwnd, ODFacet* facet )
   ...
   return (int)WinDlgBox( HWND_DESKTOP,  /* Place anywhere on desktop   */
...
MRESULT EXPENTRY ODInfoDlgProc(HWND hwndDlg, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  ...
  switch ( msg )
  {
    case WM_INITDLG:
    {
       ...
       WinSetWindowULong(hwndDlg, QWL_USER, (ULONG) facet);
    ...
    case WM_COMMAND:                    /* Posted by pushbutton or key  */
        ...
        */
       facet = (ODFacet*) WinQueryWindowULong(hwndDlg, QWL_USER);

       switch( SHORT1FROMMP( mp1 ) )     /* Extract the command value    */
       {
         case DID_OK:  /* the OK button was pressed */
         {
            /* Need to propagate this message to all of the pages. */
            ...
            /* Send on to any settings pages */
            TempODFrame frame = facet->GetFrame(ev); frame->Acquire(ev);
            TempODPart part = frame->AcquirePart(ev);
            if (part->HasExtension(ev,kODSettingsExtension)) 
            {
               ODSettingsExtension* settings = (ODSettingsExtension*)
                               part->AcquireExtension(ev,kODSettingsExtension);

               // Call Settings method to add the pages.
               ODSheet* sheets = NULL;
               ODULong sheetCount = 0;
               settings->GetNotebookSheets(ev, &sheets, &sheetCount);

               for (int i = 0; i < sheetCount; i++) 
                  WinSendMsg(sheets[i].hwnd,WM_DID_OK,NULL,NULL);
    ...
    case WM_HELP:
       {
         ...
         HWND ulCurPg = (ULONG) WinSendDlgItemMsg(hwndDlg, IDC_NOTEBOOK,
                                    BKM_QUERYPAGEWINDOWHWND, (MPARAM) ulCurPgID,
                                    (MPARAM) 0);

         WinSendMsg(ulCurPg,WM_HELP,NULL,NULL); 
       }
       break;
    ...
    case WM_CONTROL:
       {
          ...
          if (notifycode == BKN_PAGESELECTEDPENDING) 
          {
              ...
              /* page when that page is about to become the active   */
              /* page.                                               */
              /*-----------------------------------------------------*/
              ...
              if (pPageSelNotify->ulPageIdNew == ulPageIdPresentation) 
              {
                 ...
                 if (part->HasExtension(ev,kODSettingsExtension)) 
                 {
                    ...
                    // Call Settings method to get the pages.
                    ODSheet* sheets;
                    ODULong  sheetCount;
                    settings->GetNotebookSheets(ev, &sheets, &sheetCount);
       
                    // Find the corresponding sheet handle
                    ODUShort i = 0;
                    while ((i < sheetCount) && 
                         (pPageSelNotify->ulPageIdNew != sheets[i].pageId)) i++;
                    if (i < sheetCount)
                       WinSendMsg(sheets[i].hwnd,WM_SET_ACTIVE,NULL,NULL);
...
BOOL InitializeNoteBook(HWND hwnd, ODFacet * facet)
{
   ...
   // Save the window handle to the notebook dialog
   hwndParent = hwnd;
   ...
   if ( !WinSendDlgItemMsg(hwnd, IDC_NOTEBOOK, BKM_SETDIMENSIONS,
                       MPFROM2SHORT(usTabX,usTabY), /* width and height pels */
   ...
   if ( !ulPageIdPresentation) return FALSE;
   ...
   hwndPresentation = WinLoadDlg(hwnd,hwnd,(PFNWP)PresPageDlgProc,
                               (HMODULE)resInst,IDD_PRESENTATIONPAGEDLG,facet);

   if (!hwndPresentation) return FALSE;
   ...
   if ( !ulPageIdBinding) return FALSE;
   ...
   hwndBinding = WinLoadDlg( hwnd, hwnd, (PFNWP)ViewPageDlgProc,
                             (HMODULE)resInst, IDD_VIEWPAGEDLG, facet);

   if (!hwndBinding) return FALSE;
   ...
   if ( !ulPageIdDocument) return FALSE;
   ...
   hwndDocument = WinLoadDlg( hwnd, hwnd, (PFNWP)FilePageDlgProc,
                              (HMODULE)resInst, IDD_FILEPAGE1DLG, facet);

   if (!hwndDocument) return FALSE;
   ...
   // Need to acquire the part. Put it in temp object so that when
   // the temp object is out of scope, the part is released.
   TempODPart part = frame->AcquirePart(ev);

   if (part->HasExtension(ev,kODSettingsExtension))
   {
       settings = (ODSettingsExtension*)
                    part->AcquireExtension(ev,kODSettingsExtension);

       // Call Settings method to add the pages.
       settings->ShowSettingsPages(ev, facet, hwnd, IDC_NOTEBOOK);
  ...
  switch ( msg )
  {
    case WM_INITDLG:
    {
       ...
       TempODWindow window = facet->GetWindow(ev);
        ...
        // test to see if additional view types should be added - use isRootPart
	// instead of ...OfDocument so part windows are OK.
        if (!(isRootPart))
        {
...
/*---------------------------------------------------------------------*/
    ...
    case WM_COMMAND:
      if (SHORT1FROMMP( mp1 )  == DID_CANCEL)
      {
        ...
        * handle.
        */
        WinSendMsg(hwndParent, WM_COMMAND, MPFROMSHORT(DID_CANCEL),
                   MPARAM(0));
        break;
      }
    ...
    case WM_HELP:
      {
         SOM_TRY
           if (__session != kODNULL)
           {
             ...
             helpme->DisplayHelp(ev,ODHELPFILE,SET_VIEW1_PANEL);
           } /* endif */
         SOM_CATCH_ALL
         SOM_ENDTRY
         break;
    ...
    case WM_CHAR:
      {
         // To catch the accelerator for OK button, somehow notebook does
         // not generate a WM_COMMAND message as the Escape key. This is
         // a clutch to catch the 'o' or 'O' keyboard input. If the focus
         // is not in an entry field, then treat the 'o' or 'O' as the
         // accelerator for the OK button and generate a WM_COMMAND message
         // event to send back up to the parent.
         switch( SHORT1FROMMP( mp2 ) )     
         {
            case 0x186F: // for lower case o 
            case 0x006F:
            case 0x184F: // for upper case O 
            case 0x004F:
               if (WinQueryFocus(HWND_DESKTOP) != 
                   WinWindowFromID(hwndDlg, DID_PARTNAME))
               {
                  WinSendMsg(hwndParent, WM_COMMAND, MPFROMSHORT(DID_OK),
                             MPARAM(0));
                  break;
               }
         }
    ...
    case WM_HELP:
      {
         SOM_TRY
           ...
           {
             ODHelp * helpme  = __session->GetHelp(ev);
             helpme->DisplayHelp(ev,ODHELPFILE,SET_TYPE_PANEL);
           } /* endif */
         SOM_CATCH_ALL
         SOM_ENDTRY
         break;
    ...
    case WM_CHAR:
      {
         // To catch the accelerator for OK button, somehow notebook does
         // not generate a WM_COMMAND message as the Escape key. This is
         // a clutch to catch the 'o' or 'O' keyboard input. Since this page
         // does not have any entry field, treat the 'o' or 'O' as the
         // accelerator for the OK button and generate a WM_COMMAND message
         // event to send back up to the parent.
         switch( SHORT1FROMMP( mp2 ) )
         {
    ...
    case WM_HELP:
      {
         SOM_TRY
           ...
           {
             ODHelp * helpme  = __session->GetHelp(ev);
             helpme->DisplayHelp(ev,ODHELPFILE,SET_FILE1_PANEL);
           } /* endif */
         SOM_CATCH_ALL
         SOM_ENDTRY
         break;
    ...
    case WM_CHAR:
      {
         ...
         switch( SHORT1FROMMP( mp2 ) )
         {
            ...
            case 0x004F:
               if (WinQueryFocus(HWND_DESKTOP) !=
                   WinWindowFromID(hwndDlg, DID_COMMENTS))
               {
                  WinSendMsg(hwndParent, WM_COMMAND, MPFROMSHORT(DID_OK),
                             MPARAM(0));
                  break;
