...
//
// @(#) 1.19 com/src/utils/IPlfmFil.cpp, odutils, od96os2, odos29712d 3/4/97 15:39:31 [ 3/21/97 17:37:45 ]
/*
...
#if defined(_PLATFORM_WIN32_)
        // Store the fully qualify file name so as to avoid problem
        // creating by changing working directory in a file dialog.
        char *fullPathName = (char *) malloc(kODMaxFileNameSize+1);
        char *fileNameInPath;
        DWORD pathLength = GetFullPathName((char *)fileSpec->name, 
                                           kODMaxFileNameSize+1, 
                                           fullPathName, 
                                           &fileNameInPath);
        if (pathLength && pathLength <= kODMaxFileNameSize)
           strcpy((char *)fFileSpec.name, fullPathName);
        free(fullPathName);
#else
