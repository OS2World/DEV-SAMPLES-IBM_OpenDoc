...
//
// @(#) 1.9 com/src/utils/POUtils.cpp, odutils, od96os2, odos29712d 12/17/96 13:55:18 [ 3/21/97 17:36:17 ]
/*
  File:    POUtils.cpp
...
{
#if (defined _PLATFORM_WIN32_ || defined _PLATFORM_UNIX_ || defined _PLATFORM_OS2_)
  if (su->Exists(ev, kODPropModUser, kODPlatformIText, 0) == kODFalse) {
    ODSUForceFocus(ev, su, kODPropModUser, kODPlatformIText);
    UpdateUserName(ev, su);
  }
#elif (defined _PLATFORM_MACINTOSH)
...
{
#if (defined _PLATFORM_WIN32_ || defined _PLATFORM_UNIX_ || defined _PLATFORM_OS2_)
  ODIText* userName = ODGetITextProp(ev, fromSU, kODPropModUser, 
                                     kODPlatformIText, kODNULL);
  if (userName)
    ODSetITextProp(ev, toSU, kODPropModUser, kODPlatformIText, userName);
#elif (defined _PLATFORM_MACINTOSH)
  ODIText* userName = ODGetITextProp(ev, fromSU, kODPropModUser, 
                                     kODMacIText, kODNULL);
  if (userName)
...
#if (defined _PLATFORM_WIN32_ || defined _PLATFORM_UNIX_ || defined _PLATFORM_OS2_)
  ODSetITextProp(ev, su, kODPropModUser, kODPlatformIText, userName);
#elif (defined _PLATFORM_MACINTOSH)
...
{
#if (defined _PLATFORM_WIN32_ || defined _PLATFORM_UNIX_ || defined _PLATFORM_OS2_)
  ODIText* partName = ODGetITextProp(ev, fromSU, kODPropName, kODPlatformIText, 
                                     kODNULL);
  if (partName && GetITextStringLength(partName))
    ODSetITextProp(ev, toSU, kODPropName, kODPlatformIText, partName);
#elif (defined _PLATFORM_MACINTOSH)
  ODIText* partName = ODGetITextProp(ev, fromSU, kODPropName, kODMacIText, 
                                     kODNULL);
  if (partName && GetITextStringLength(partName))
...
{
#if (defined _PLATFORM_WIN32_ || defined _PLATFORM_UNIX_ || defined _PLATFORM_OS2_)
  ODIText* partComment = ODGetITextProp(ev, fromSU, kODPropComments, 
                                        kODPlatformIText, kODNULL);
  if (partComment && GetITextStringLength(partComment))
    ODSetITextProp(ev, toSU, kODPropComments, kODPlatformIText, partComment);
#elif (defined _PLATFORM_MACINTOSH)
  ODIText* partComment = ODGetITextProp(ev, fromSU, kODPropComments, 
                                        kODMacIText, kODNULL);
  if (partComment && GetITextStringLength(partComment))
