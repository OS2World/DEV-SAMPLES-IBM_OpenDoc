...
//
// @(#) 1.16 com/src/storage/Clipbd.cpp, oddataxfer, od96os2, odos29712d 3/20/97 14:08:00 [ 3/21/97 17:21:22 ]
/*
	File:		Clipbd.cpp
...
//                    we've imported the clipboard, so we must have the latest changes
                // rrk 20mar97 - if we are read-only, don't export a linkspec
                ODWindow *window = _fSession->GetWindowState(ev)->AcquireActiveWindow(ev);
                if(window) {
                   ODBoolean isDraftReadOnly = (window->GetDraft(ev)->GetPermissions(ev) == kODDPReadOnly);
                   //PRINT("ExportClipboard: isDraftReadOnly is %d.\n", isDraftReadOnly);
                   if (isDraftReadOnly)
		      if ( _fSU->Exists(ev, kODPropLinkSpec, kODNULL, 0) )
	        ...
	        if ( _fExportedLinkSpec )
		{
			needToExportToRemoveLinkSpec = _fSU && !_fSU->Exists(ev, kODPropLinkSpec, kODNULL, 0);
                   ...
                   if((_fLastImportUpdateID == _fUpdateID) && !needToExportToRemoveLinkSpec) // we're in sync
                     return;
                }
