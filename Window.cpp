/* @(#)Z 1.52 com/src/ui/Window.cpp, odui, od96os2, odos29712d 97/03/21 17:20:40 (97/02/24 13:12:52) */
...
SOM_Scope void  SOMLINK ODWindowInitWindowTitle (ODWindow *somSelf, Environment *ev)
{
    ...
    SOM_TRY
        ...
        if (!_fIsRootWindow)
        {
	  // Make sure we have a non-null sourceframe. If it is NULL then it is a part owned
          // window and we don't want to put a title on it.
	  if (_fSourceFrame) {
                ...
                ODEventData* theEvent)
{
    ...
    ODWindowMethodDebug("ODWindow","HandleUpdateEvent");
        ...
        // Call the update method passing in theEvent as a parameter
                somSelf->UpdateWindow(ev, theEvent);
#endif
        SOM_CATCH_ALL
        SOM_ENDTRY
}
...
SOM_Scope void  SOMLINK ODWindowUpdateWindow(ODWindow *somSelf,  Environment *ev,
                                             ODEventData* theEvent)
{
    ODWindowData *somThis = ODWindowGetData(somSelf);
    ODWindowMethodDebug("ODWindow","Update");
    ...
    SOM_TRY
                ...
                if (_fPlatformWindow)
                {
                    // #1248624 Failure handling should be further improved,
                    // but this should prevent the infinite dialog problem
#ifdef _PLATFORM_UNIX_
