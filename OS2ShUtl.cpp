/* @(#)Z 1.62 os2/src/docshell/OS2ShUtl.cpp, odshell, od96os2, odos29712d 97/03/21 17:40:54 (97/03/18 14:19:41) */
...
//------------------------------------------------------------------------------
// DraftHistoryDlg - invokes the Draft History dialog.
//------------------------------------------------------------------------------
SOMEXTERN void SOMLINK DraftHistoryDlg(Environment *ev, DraftWindow* dwThis)
{
  ODBoolean cleanup_needed = kODTrue;
  ...
  try
  {
    ...
    EnableApplModal(ev, dwThis->GetWindow(), FALSE);

    // If there are drafts display the draft history dialog.
    if (dwThis->GetDraftInfoList()) 
    {
        WinDlgBox(HWND_DESKTOP, dwThis->GetWindow(), (PFNWP) DraftDialogFunc, 
                  dwThis->GetInstance(), IDD_DRAFTHISTORYDLG, (PVOID) dwThis);
    }
    else
    {
        WinMessageBox(HWND_DESKTOP,dwThis->GetWindow(),
                      ...
                      catgets(ODcat_handle,DOCSH_SET,DOCMSG_2,
                        "Information"), 0, MB_INFORMATION | MB_OK);
    }

    EnableApplModal(ev, dwThis->GetWindow(), TRUE);

    // If we get to this point without any exceptions, set flag
    // so that if we except in the deletion of dwThis, we don't   
    // attempt to delete it again in the catch block.
    cleanup_needed = kODFalse;

    delete dwThis; 
  }
  catch (ODException _exception)
  {
     LOG("Exception occured in DraftHistoryDlg\n");
     RestoreApplModal(dwThis->GetWindow());
...
SOMEXTERN void SOMLINK AboutDlg (Environment *ev)
{
      ...
      WinMessageBox(HWND_DESKTOP, hwnd,
                 ...
                 "         IBM OpenDoc(R) Version 1.2\n\n(c) Copyright IBM 1997, all rights reserved.\nOpenDoc is a trademark of Apple Computer Inc.,\nregistered in the United States and other countries."),
  ...
  MPARAM       draftInfo;
    ...
    switch (msg)
    {
	  ...
	  WinSetWindowULong(hwnd, QWL_USER, (ULONG) draftInfo);
          ...
          // Set limit for text entry in fields.
          WinSendDlgItemMsg(hwnd, DID_DRAFTNAME, EM_SETTEXTLIMIT, 
                            (MPARAM) kODMaxNameLength, (MPARAM) 0);
          WinSendDlgItemMsg(hwnd, DID_DRAFTCOMMENT, EM_SETTEXTLIMIT, 
                            (MPARAM) kODMaxLength, (MPARAM) 0);
      ...
      case WM_COMMAND:
      {
        ...
        switch (SHORT1FROMMP(mp1))
        {
          ...
          case DID_SAVE:
          {
              ...
              WinQueryDlgItemText(hwnd, DID_DRAFTNAME, kODMaxNameLength,
  ...
  try
  {
    ...
    switch (message)
    {
      ...
      case WM_INITDLG:
      {
          ...
          // If for some reason dwThis isn't received, return.
          if (!dwThis) break;
          ...
          { 
            ...
            char          userName[kODMaxNameLength], comments[kODMaxLength];
                 ...
                 pDraftInfo = pDraftInfo->GetNext())
            {
                ...
                if (time = pDraftInfo->GetSavedDate())
                {
                  ...
                  // initialize structures that will be used by the recordcores.

                  // Because tm_mon is defined to be the range from 0-11,
                  // we need to add 1 to have the accurate month.
                  cdate.month = (UCHAR) (timeptr->tm_mon + 1);
                  cdate.day = (UCHAR) timeptr->tm_mday ;
                  cdate.year = (ODUShort)timeptr->tm_year;
                  ctime.hours = (UCHAR) timeptr->tm_hour;
                  ctime.seconds = (UCHAR) timeptr->tm_sec;
                  ctime.minutes = (UCHAR) timeptr->tm_min;
      ...
      case WM_CONTROL:
      {
          ...
          if (SHORT1FROMMP(mp1) == DID_DRAFTCNTR)
          {
	          ...
	          PNOTIFYRECORDENTER pn = (PNOTIFYRECORDENTER) mp2;
                  ...
                  if (!(dwThis->OpenDraft(draftNum, NULL)))
      ...
      case WM_COMMAND:
      {
	      ...
	      PODINFO pTmpRecord = 
	         (PODINFO) PVOIDFROMMR(WinSendMsg(DraftWnd, 
              ...
              if (!dwThis->OpenDraft(pTmpRecord->draftNumber,NULL))
                 WARNMSG(WARN_INDEX(AMSG_890),
                         "CPPOD0402 - Unable to open draft.");
              else
              {
		...
		WinEnableWindow(hdwnd, FALSE);
                ...
                if (dwThis->RemoveDraft(pTmpRecord->draftNumber, NULL))
                {
...
//------------------------------------------------------------------------------
// ReadOnlyDlg - brings up an alert dialog to warn user that the document
//                   is read-only and ask user whether to continue or not.
//------------------------------------------------------------------------------
SOMEXTERN ODBoolean SOMLINK ReadOnlyDlg(Environment *ev)
{
    ...
    try
    {
       ULONG ulResult = WinMessageBox(HWND_DESKTOP, 
                         shell->GetWindowSystemData(ev)->parentWindow, 
                         catgets(ODcat_handle, DOCSH_SET, DOCMSG_100, 
	         	    "You are about to open a read-only document and will not be able to make changes to it. Do you want to open the document anyway?"),
	    	         catgets(ODcat_handle, DOCSH_SET, DOCMSG_101,"Open Confirmation"),
	    	         0, MB_ICONQUESTION | MB_YESNO);
   ...
   {
      LOG("OpenDoc exception occured in ReadOnlyDlg.\n");
      throw;
   }
   return ret;
}
