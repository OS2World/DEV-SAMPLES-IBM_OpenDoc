/* @(#)Z 1.20 os2/src/docshell/OS2Shell.cpp, odshell, od96os2, odos29712d 97/03/21 17:40:54 (97/01/03 11:49:45) */
                                               ...
                                               Environment *ev)
{
    ...
    {
       LOG("SOM exception occured in OS2Shell::CloseCleanup.\n");
       ODSetSOMException(ev, _exception);
    }
}
...
// DispatchEvent
//
//      Forward the event the the dispatcher but look for and act upon
//      events that are of interest to us.
//------------------------------------------------------------------------------
SOM_Scope ODBoolean  SOMLINK OS2ShellDispatchEvent(OS2Shell *somSelf, 
                                                   Environment *ev, 
                                                   ODEventData* event)
{
    OS2ShellMethodDebug("OS2Shell","OS2ShellDispatchEvent");
    ...
    try
    {
        ...
        {
            // For part using OpenClass, window closing caused OpenClass frame 
            // handler sends WM_SYSCOMMAND with SC_CLOSE to the client 
            // window. In order to take care of closing from OpenClass, need
            // to handle this special case. Otherwise, send the rest to
            // the parent's DispatchEvent.
            ...
            default:
            {
                handled = OS2Shell_parents_DispatchEvent(somSelf, ev, event);
            }
        }
    }
    ...
    {
        LOG("SOM exception occured in OS2Shell::DispatchEvent.\n");
        ODSetSOMException(ev, _exception);
    }
