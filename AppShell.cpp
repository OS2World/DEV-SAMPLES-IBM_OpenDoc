/* @(#)Z 1.86 com/src/docshell/AppShell.cpp, odshell, od96os2, odos29712d 97/03/21 17:37:28 (97/03/05 14:35:22) */
                              ...
                              char *partKind, ODUShort actionFlag)
{
    ...
    try
    {
        ...
        if (selInfo != kODCommandCancel)
        {
            ...
            if (fileName[0] != '\0')
            {
                // Open the document for regular document. If document is
                // read-only and user decides to open the document, then
                // enter the main loop, else don't enter the main loop.
                if (!(docMgr->OpenDocumentFromFile(ev, fileName, actionFlag)))
