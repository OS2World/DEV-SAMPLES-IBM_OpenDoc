/* @(#)Z 1.23 com/src/ui/StdDispO.cpp, odui, od96os2, odos29712d 97/03/21 17:37:42 (97/03/05 16:03:02) */
...
void OS2DispatchModule::InitOS2DispatchModule(Environment* ev, ODSession* session)
{
  ...
  fPointer = fDefaultPointer =  WinQuerySysPointer(HWND_DESKTOP, SPTR_ARROW, FALSE); 
  fActiveBorderPointer = WinQuerySysPointer(HWND_DESKTOP, SPTR_MOVE, FALSE);
}
					...
					ODEventInfo* eventInfo)
{
    ...
    {
      case WM_MOUSEMOVE         : 
        handled = this->DispatchMouseEvent(ev, event, eventInfo);
        if (!handled) {
          SetMousePointer(ev, event, eventInfo);
          handled = kODTrue;
        }
        return handled;
...
// SetMouseCursor                                                                                                                                                         
// ======================================================================                                                                                                 
void OS2DispatchModule::SetMousePointer(                                                                                                              
			   Environment *ev,                                                                                                                                               
			   ODEventData *event,                                                                                                                                            
			   ODEventInfo *eventInfo)                                                                                                                                        
{                                                                                                                                                                         
  ...
  // Assign the where structure prior to SendToFocusOwner                                                                                                                 
  ODPoint windowPt;
  windowPt.x = ODIntToFixed(SHORT1FROMMP(event->mp1));
  windowPt.y = ODIntToFixed(SHORT2FROMMP(event->mp1));
  eventInfo->where.x = windowPt.x;
  eventInfo->where.y = windowPt.y;

  // Check to see if we are over an active border or not. If so, change the cursor                                                                                        
  ODFacet* activeFacet = kODNULL;                                                                                                                                         
  HWND platformWindow = WinQueryWindow(event->hwnd, QW_PARENT);
  TempODWindow theWin = fWindowState->AcquireODWindow(ev, platformWindow);
  ODFrame* activeFrame = fArbitrator->AcquireFocusOwner(ev, fSelectionFocusToken);                                                                                      
  if (activeFrame )                                                                                                                                                       
    activeFacet = GetActiveFacetWithBorderUnderPoint(ev,                                                                                                                  
						     activeFrame,                                                                                                                                                   
						     theWin,                                                                                                                                                        
						     (eventInfo->where));                                                                                                                                           
                                                                                                                                                                          
  // if true the mouse is over an active border so set fPointer to the active border cursor.                                                                               
  if ( activeFacet )                                                                                                                                                      
    WinSetPointer(HWND_DESKTOP, fActiveBorderPointer);                                                                                                                                      
                                                                                                                                                                          
  // else use the default cursor.                                                                                                                                         
  else                                                                                                                                                                    
    WinSetPointer(HWND_DESKTOP, fDefaultPointer);                                                                                                                                           
