/* @(#)Z 1.62 com/src/ui/MenuBar.cpp, odui, od96os2, odos29712d 97/03/21 17:20:19 (97/03/21 13:30:37) */
...
{
    ODMenuBarData *somThis = ODMenuBarGetData(somSelf);
    // It looks like Windows does this for us automatically.  Don't really
    // need to to call the Win32 DrawMenuBar API
    return; 
                                                    ...
                                                    Environment *ev)
{
	...
	// rrk - Since we are restoring, don't add any.
	...
	somSelf->ChangeAccelTable(ev);
	    ...
	    // ----------------------------------------------------------------------
	    // Actually, we don't need to do anything here at all.  The call to ChangeAccelTable
	    // we just completed restored the default accelerator table for us.  If we run through
	    // the following code, we can get into big trouble.  This code assigns _fDefaultAccelTable
	    // to fNewAccelTable.  fNewAccelTable is volatile, and gets destroyed at the drop of a
	    // had.  fDefaultAccelTable had better stick around for ever.	    
#if 0
			  _fNewAccelTable,
			  _fDefaultAccelTable);
	    if (_fNewAccelTable)
		...
		{
		    PRINT("MenuBarRestoreAccelTable: destroying table %x to restore %x.\n", 
			  _fNewAccelTable,
			  _fDefaultAccelTable);
                                                   ...
                                                   Environment *ev)
{
	...
	newCols = ulCols + _fNumberofAccels;
	    // rrk - don't want to zero until a "restore" is done.
#if 0
	_fNumberofAccels = 0;
#endif
	...
	if (_fAccelsToAdd)
	{
	    ...
	    // rrk - don't want to set to false until a "restore" is done.
#if 0
	    _fAccelsToAdd = kODFalse;
#endif
	}
...
//-------------------------------------------------------------------------------------
// GetMenuAndItem [OS/2, WIN32, AIX only]
//-------------------------------------------------------------------------------------
SOM_Scope void  SOMLINK ODMenuBarGetMenuAndItem(ODMenuBar *somSelf, Environment *ev,
                ODCommandID command,
                ODMenuID* menu,
                ODMenuItemID* menuItem)
{
	...
	*menuItem = NULL;
	  ...
	  // Get a list of all the submenuIDs.  For each one, see if our menu item is
	  // under that submenu.
	    ...
	    if (GetSubMenuIDs(somSelf, ev, &nSubMenuIDs, &subMenuIDs))
	    {
	      ...
	      for(int j = 0;	 ((!found) && ( j< nSubMenuIDs));      j++) {

		if (GetWinMenuAdapter(somSelf, ev, subMenuIDs[j].id, &winMenuAd))
		  if (found = winMenuAd->ItemExists(ev, command)) {
		    *menu = subMenuIDs[j].id;
		    *menuItem = (ODMenuItemID) command;
		  }
	      }
	      delete subMenuIDs;
	    }
	    ...
	    ODOS2MenuItemAdapter* itemAd;
            ...
            for ( ODOS2MenuAdapter* menuAd = (ODOS2MenuAdapter*) iter.First();
	          ...
	          menuAd = (ODOS2MenuAdapter*) iter.Next() )
            {
	        if( menuAd->GetMenuItemAdapter( ev, command, &itemAd ) ) {
	            found = kODTrue;
	            *menu = (itemAd->parentAdapter)->MenuID( ev );
                    *menuItem = (ODMenuItemID) command;
	        }
            }
...
SOM_Scope void  SOMLINK ODMenuBarsomUninit(ODMenuBar *somSelf)
{
    ...
    // RRK - don't think we want this code.  If we are indeed the current menu bar,
    // then calling SetCurrentMenuBar with the second parameter = NULL will result in
    // a recursive call to somUninit.
#if 0
    // ----------------------------------------
    if (_fSession)
    {
                                          ...
                                          Environment *ev)
{
	...
	if (somSelf->GetRefCount(ev) == 0)
	    somSelf->somFree();
    #endif // _PLATFORM_MACINTOSH or WIN32
