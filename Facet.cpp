/* @(#)Z 1.100 com/src/layout/Facet.cpp, odlayout, od96os2, odos29712d 97/03/21 17:18:25 (97/01/30 16:50:51) */
...
//------------------------------------------------------------------------------
// ODFacet: CopyFacetClipRegion
// This method is the same as CopyAggregateClipRegion but does not subtract out
// the facet's children's shape only the siblings' shape.
//------------------------------------------------------------------------------

SOM_Scope ODRgnHandle SOMLINK ODFacetCopyFacetClipRegion(ODFacet *somSelf, Environment *ev, HPS hps)
{
    ODFacetData *somThis = ODFacetGetData(somSelf);
    ODFacetMethodDebug("ODFacet","CopyAggregateClipRegion");
...
//------------------------------------------------------------------------------                     
// ODFacet:ChildUnderPoint                                                                     
//------------------------------------------------------------------------------                     
                                                                                                     
SOM_Scope ODPlatformWindow SOMLINK ODFacetChildUnderPoint(ODFacet *somSelf, Environment *ev, ODPoint *odPt) 
{
   HWND hwndTarget = kODNULL;
   HWND hwndFacet = somSelf->GetFacetHWND(ev);                                                                                    
   if (hwndFacet)                         
   {                                                                                                                            
         POINTL ptl = odPt->AsPOINTL();                                                                                          
         WinSetWindowULong(hwndFacet, QWL_HITTESTFACET, TRUE); // Tell facet window not                                             
                                                                                                 //  to be transparent.                                                              
         hwndTarget = WinWindowFromPoint(hwndFacet, &ptl, TRUE);                                                               
           ...
           if ( myCanvas->IsOffscreen(ev) )
           {
               ...
               parentCanvasFacet->Invalidate(ev, tShape, kODNULL);
                 ...
                 WinInvalidateRegion(hwnd, tShape->GetRegion(ev), FALSE);
                   ...
                   // Copy over the current tShape
                   TempODShape tShape2 = tShape->Copy(ev);

                   // Transform the shape - it's now in window coordinates.
                   // We'll need this in logic below.              
                   tShape2->Transform(ev, xform);
                   ...
                   if ( myCanvas->IsDynamic(ev) )
                   {
                       ...
                       // AIX excludes the child widget regions from the
                       // clear above, so we have to invalidate the
                       // child facets explicitly.  This logic makes
                       // AIX consistent with Mac, OS/2 and NT. 

                       // For all the immediate embedded child facets
                       // of this facet
                       ODFacetIterator* i;
                       i = somSelf->CreateFacetIterator(ev, kODChildrenOnly,
                                                        kODBackToFront);
                       ...
                       for (ODFacet *facet = i->First(ev);
                            ...
                            facet = i->Next(ev))
                       {
                           // Acquire window frame transform of child
                           ODTransform *xform3;
                           xform3 = facet->AcquireWindowFrameTransform(ev,
                                                                       kODNULL);

                           // Copy over the tShape2 above. The tShape2 is
                           // already in window coordinates.
                           TempODShape tShape3 = tShape2->Copy(ev);

                           // Invert the shape to be in the embedded facet's
                           // frame coordinate system
                           xform3->InvertShape(ev, tShape3);

                           // Call the child facet's invalidate. There is no
                           // reason to check if the embedded facet's
                           // aggregateclipshape intersects with the invalid
                           // shape being passed in as that is performed first
                           // thing in the Invalidate method.  Having it here
                           // would be redundant code.
                           facet->Invalidate(ev, tShape3, biasCanvas);
                       }
