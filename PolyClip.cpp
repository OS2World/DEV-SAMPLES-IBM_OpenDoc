...
/* @(#)Z 1.12 com/src/imaging/polyclip/PolyClip.cpp, odimaging, od96os2, odos29712d 97/03/21 17:18:23 (97/02/05 14:19:22) */
...
PolyOutset( ODPolygon &poly, ODCoordinate dist )
{
  ...
  // On OS/2, the distance needs to be negated because of the differences in
  // the direction of the +Y axis when calculating points perpendicular to
  // the original points.
  dist = -dist;
#endif
...
// AdjustPolygon does the actual work in walking
// the set of points that represent the contour
// and eliminating the point at location i+1 in the contour.
// A new count of points in the contour is returned.
int 
...
// SimplifyPoly reduces the number of vertices in a contour
// by eliminating co-linear points.
int 
...
//  }
   ...
   for (int i = 0; i < n; i++) {
      ...
      if ( (p0.x == p1.x) && (p0.y == p1.y) ) {
         ...
         double seg1DeltaX = p1.x - p0.x;
         double seg2DeltaX = p2.x - p1.x;

         if ( seg1DeltaX == 0.0 ) {
            // 1st segment is vertical, check the 2nd
            if ( seg2DeltaX == 0.0 ) {
               // must be 2 vertical line segments, p1 is co-linear
               n = AdjustPolygon(p, i, n);
               i--;
            }
         }
         else {
            // If we get here, we know that segment 1 is not vertical.
            // If segment 2 is vertical then we know that p1 is not
            // co-linear.  If it isn't vertical then we need to
            // calculate the slope for both segments and compare them.
            if ( seg2DeltaX != 0.0 ) {
               double slope1 =  (p1.y - p0.y) / seg1DeltaX;
               double slope2 =  (p2.y - p1.y) / seg2DeltaX;
...
PolySimplify( ODPolygon &dstPoly, const ODPolygon &srcPoly )
{
  ...
  // We'll track 2 pointers here.  The SimplifyPoly method reduces
  // the number of vertices within a contour. If we have to toss some
  // vertices because they are co-linear we need one pointer
  // to use as an index into the original contour definitions
  // and one to point to the current position of the new definition.  
  // Note that we'll never have to realloc memory since the contours 
  // will only decrease in number, never increase.
  ODContour *cont = (ODContour *)srcPoly.FirstContour();
  ODContour *origCont = (ODContour *)srcPoly.FirstContour();
  ODBoolean beenSimplified = kODFalse;

  for( int i = srcPoly.GetNContours(); i > 0; i-- ) {
    int n = origCont->nVertices;
    SimplifyPoint *p = (SimplifyPoint *)malloc(n * sizeof(SimplifyPoint));
    for( int k = 0; k < n; k++ ) {
      p[k].x = (double)ODFixedToFloat(origCont->vertex[k].x);
      p[k].y = (double)ODFixedToFloat(origCont->vertex[k].y);
    }
    int newn = SimplifyPoly(p, n);

    origCont = origCont->NextContour();

    if ( (newn != n ) || ( beenSimplified == kODTrue ) ) {
      beenSimplified = kODTrue;
      cont->nVertices = newn;
      for( k = 0; k < newn; k++ ) {
      ...
      cont = cont->NextContour();
       ...
       // So far, the contours have not been simplified.
       // Our 2 pointers should point to the same contour.
       cont = origCont;
    }
...
//
/***************************************************************************/
//* DumpPolys - Dumps platform specific region data
/***************************************************************************/
static void
DumpPolys( long nPolys, const ODPolygon* polys[] )
{

   cout << endl;
   cout << "DumpPolys: printing " << nPolys << " polygons." << endl;
  
   for (int i = 0; i < nPolys; i++ ) {
      const ODPolygon &p = *polys[i];
      cout << "  Polygon " << i << "has  " << p.GetNContours << " contours." << endl;
      const ODContour *c = p.FirstContour();
      for( int j = 0; j < p.GetNContours(); j++ ) {
        cout << "    Contour " << j << "has  " << c->nVertices << " vertices." << endl;
        for( int k = 0; k < c->nVertices; k++ ) {
          cout << "        " <<  (float)ODFixedToFloat(c->vertex[k].x) << " , " << (float)ODFixedToFloat(c->vertex[k].y) << endl;
        }
        c = c->NextContour();
      }
    }
