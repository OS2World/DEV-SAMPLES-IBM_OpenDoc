/* @(#)Z 1.45 com/src/bento/CMDraft.cpp, odstorage, od96os2, odos29712d 97/03/21 17:19:03 (97/03/03 14:38:44) */
    ...
    CMDraftMethodDebug("CMDraft","somUninit");
     ...
     try
     {
       ...
       parent_somUninit(somSelf);
                ...
                // Set to NULL so that PartDeleted method is not going
                // to try to access _fPartClasses.
                _fPartClasses = kODNULL; 
    ...
    CMDraftMethodDebug("CMDraft","BeginClone");
        ...
        if(destDraft==kODNULL)
           THROW(kODErrInvalidParameter);
                ...
                {
//                 THROW(kODErrCloningInProgress);
                   // The sleep loop will allow multi-threads entering this same mehtod
                   // by gating each thread's operation.
                   // defect 27471
                   while (_fLockCount > 0) {
                          DosSleep(1000);
                         } /* endwhile */
                ...
                ODID scopeID)
{
    ...
    try
    {
        ...
        case kODCloneDropCopy:
                ...
                break;
        case kODCloneDropMove:
                // Putting DR4 cloning code to handle drag and drop within the
                // same document.
                if ( GetOriginalDraft(ev, somSelf) == _fDestDraft )
                {
                ...
                ODPart* realPart)
{
    ...
    CMDraftMethodDebug("CMDraft","PartDeleted");
        ...
        ODULong count = 0;
        // Check if _fPartClasses is valid before accessing it
        if ((_fPartClasses) && (_fPartClasses->GetValue(&partClass, &count))) 
        {
                if (count > 0) {
                        --count;
                        _fPartClasses->ReplaceEntry(&partClass, &count);
