...
// 
// @(#) 1.22 com/src/utils/WinUtils.cpp, odutils, od96os2, odos29714c 3/21/97 10:40:59 [ 4/2/97 17:19:32 ]
/* 
        File:           WinUtils.cpp 
...
{ 
        ...
        { 
			...
			// Determine the draft number. If == 0 then this is the original window else it is a
			//  draft window. If it is a draft window then offset it from the original window.
			{
			  // Retrieve draft's properties
			  TempODStorageUnit DraftProperties = frameSU->GetDraft(ev)->AcquireDraftProperties(ev);
			  ...
			  if (DraftProperties->Exists(ev,
						      ...
						      0))
			    {
			      ...
			      int id;
			      if ((id = ODGetULongProp(ev, DraftProperties, kODPropDraftNumber, kODULong)) != 0) {

                                ODFixed screenHeight, screenWidth;

#ifdef _PLATFORM_UNIX_
                                Screen *fScreen = frameSU->GetDraft(ev)->GetDocument(ev)->GetContainer(ev)->GetStorageSystem(ev)->GetSession(ev)->GetWindowState(ev)->GetScreen(ev);
                                screenHeight = ODIntToFixed(HeightOfScreen(fScreen));
                                screenWidth = ODIntToFixed(WidthOfScreen(fScreen));
#elif _PLATFORM_WIN32_
                                ...
                                DevQueryCaps(hdc, CAPS_HEIGHT, 1, &sHeight);
                                DevQueryCaps(hdc, CAPS_WIDTH, 1, &sWidth);
                                WinReleasePS( hps );
                                screenHeight = ODIntToFixed(sHeight);
                                screenWidth = ODIntToFixed(sWidth);
#endif

                                if (((odRect.left + ODIntToFixed(10*id)) > screenHeight) ||
                                    ((odRect.top + ODIntToFixed(10*id)) > screenWidth))
                                {
                                    odRect.right -= odRect.left;
                                    odRect.right += ODIntToFixed(10*id);
                                    odRect.left = ODIntToFixed(10*id);
                                    odRect.bottom -= odRect.top;
                                    odRect.bottom += ODIntToFixed(10*id);
                                    odRect.top = ODIntToFixed(10*id);
                                }
                                else
                                {
                                    odRect.left += ODIntToFixed(10*id);
